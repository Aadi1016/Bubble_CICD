...
def auth_call ():
    auth_url = 'https://ps-internal.thoughtspot.cloud/api/rest/2.0/auth/token/full'
    ...
    payload = {
        ...
        "auto_create": False,
        "password": "Planet@107",
        "secret_key": "69ccf723-01ec-4d37-9f38-b0f29dde5d63"
    }
...
def commit_call(Bearer_token):
    ...
    metadata_search_url = 'https://ps-internal.thoughtspot.cloud/api/rest/2.0/metadata/search'
    # Version Control API url for COMMIT 
    commit_url = 'https://ps-internal.thoughtspot.cloud/api/rest/2.0/vcs/git/branches/commit'
    ...
    payload = {
        ...
        "metadata": [
            ...
            "tag_identifiers": ["version-controlled"],
            
    }

    try:
        response = requests.post(metadata_search_url, headers=headers, data=json.dumps(payload))
        response.raise_for_status()  # Raises error if status >= 400
    ...
    for i in range(len(metadata_df)) :
        ...
        payload = {
            ...
            "delete_aware": True,
            "branch_name": "ts-dev",
            "comment": "This is commiting the tmls to the ts-dev branch"
        }
